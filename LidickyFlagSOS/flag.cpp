...
CFLAGS= -DG_COLORED_EDGES=2  -DG_USE_LEXMIN_FOR_ISOMORPHISM -DG_COLORED_VERTICES_SYMMETRIC
SRC=../flag.cpp
...
#define G_COLORED_EDGES_PARTITION //       1 is non-edge and the other denote edge partiton. Similar as G_COLORED_EDGES_BLIND up to one color 1 but more efficient, especially for more colors
#define G_COLORED_EDGES_PARTITION_NONEDGE // 1 as non-edge is ignored. All is partition
//#define G_COLORED_EDGES_BLIND_UP_TO_1    // this is for generating graphs where we can assume the used colors need to be at least consecutive?
#define G_COLORED_EDGES_SYMMETRIC //       Similar to blind version but the symmetry is applied only during SDP generation by combining symmetric graphs on Kn vertices
#define G_COLORED_EDGES_SYMMETRIC_CONSTRAINTS  adds constraints for edge symmetries
...
#define G_ORIENTED_EDGES_SYMMETRIC //      Same as G_ORIENTED_EDGES_BLIND but applied only when bilding the SDP
...
#define G_LOW_MEMORY_FLAG_GENERATING // use less memory when generating flags - it is slower but it works for larger graphs
 ...
 */
 ...
 #define G_USE_LEXMIN_FOR_ISOMORPHISM
 #endif
 #endif


 #ifdef G_USE_LEXMIN_FOR_ISOMORPHISM_OLD
 #ifndef G_USE_LEXMIN_FOR_ISOMORPHISM
 #define G_USE_LEXMIN_FOR_ISOMORPHISM
 #endif
 #endif
...
#ifndef G_USE_LEXMIN_FOR_ISOMORPHISM_OLD
...
void print_backtrace()
{
    ...
    if (strings == nullptr) {
        std::cerr << "Failed to obtain backtrace.\n";
        return;
    }
    std::cerr << "Backtrace:\n";
...
#ifdef G_COLORED_EDGES
// For each size and each [x,y,z] it stores index
// in a linear ordered field for storage of size
// (V choose 3) = V*(V-1)*(V-2)/6
...
public:
    ...
    {
#ifdef G_ORDERED_VERTICES
        return m_Theta == m_labeled_vertices_cnt;
#else
...
public:
    ...
    bool is_isomorphic_to_colorblind_colored_edges(const flag &h, bool allow_symmetry = false) const
    {
        ...
        for (int c = 0; c < COLORS_EDGES; c++)  // TODO ask chatgpt for one line
        {
            classes_mapped_to_this.push_back(false);
            class_permutation_for_h.push_back(0);
        }

        // The first classe 0 if fixed 
    ...
    bool is_isomorphic_to(const flag &h, bool allow_symmetry=false) const
    {
        ...
        {
#ifdef G_USE_LEXMIN_FOR_ISOMORPHISM_STR
            // if we have signatures, we are can just use them and that is it
            if (!m_minlex_signature_str.empty() && !h.m_minlex_signature_str.empty())
            {
#ifdef G_PROFILING
                #pragma omp atomic
                g_prof_min_lex_signatures_isomorphic_tests++;
#endif
                if (m_minlex_signature_str.compare(h.m_minlex_signature_str) != 0) return false;
                return true;
            }
#endif
...
*/
...
#ifdef G_USE_LEXMIN_FOR_ISOMORPHISM
#ifdef G_USE_LEXMIN_FOR_ISOMORPHISM_STR
        m_minlex_signature_str.clear();
        //m_minlex_signature_str = "X";
#endif
    ...
    {
#ifdef G_USE_LEXMIN_FOR_ISOMORPHISM_STR
        if (m_minlex_signature_str.empty() || h.m_minlex_signature_str.compare(m_minlex_signature_str ) < 0)
        {
            m_minlex_signature_str = h.m_minlex_signature_str;
            return true;
        }
#endif
...
#ifdef G_USE_LEXMIN_FOR_ISOMORPHISM_STR
    //cerr << "str: " <<  m_minlex_signature_str << endl;
#endif
...
#define ADD_TO_SIG(X) {  assert(SIGNATURE_LENGTH > id); sig[id] = X; if (sig_is_better == false) { if (sig[id] > m_minlex_signature[id]) goto endOfLoop; if (sig[id] < m_minlex_signature[id]) sig_is_better = true; } id++; }
    ...
    #ifdef G_COLORED_EDGES
                for (int u = 0; u < v; u++)
                    ADD_TO_SIG(get_2edge(permV[u],pv));
    #endif
    ...
    void create_minlex_signature_WIP2()
    {
        //cerr << "WIP2 for " << print() << endl;
       ...
       for (int i = 0; i < SIGNATURE_LENGTH; i++)
		...
		int permV[V];
		for (int i = 0; i < m_vertices; i++) permV[i] = i;
		//std::sort (permV,permV+m_vertices);
    	...
    	do {
            ...
            if (last_v_tested == m_vertices-1)
            {
                if (!std::next_permutation(permV+m_Theta,permV+m_vertices)) break;
                continue;
            }

            int next_to_v=m_vertices;
            int next_to_v_ID = -1;
            for (int i = last_v_tested+1; i < m_vertices; i++)
            {
                if (permV[i] > permV[last_v_tested] && permV[i] < next_to_v)
                {
                    next_to_v = permV[i];
                    next_to_v_ID = i;
                }
            ...
            std::swap(permV[last_v_tested], permV[next_to_v_ID]);
            std::sort(permV+last_v_tested+1, permV+m_vertices);
		} while ( true );

        //cerr << "test_counter=" << test_counter << endl;
...
//		} while ( std::next_permutation(permV+m_Theta,permV+m_vertices) );
    ...
    // sig is an array,  sig_length is how long the returned array is
    void extend_sig(bool &sigvalid, flag_t *sig, int &sig_length, int next_p, int *permV, int *usedV, flag_t *sig_test,flag_t *sig_best, int buffer_size)
    {
        int  sig_best_length=0;
        //int  sig_test_length;

        cerr << "next_p=" << next_p << " permV=";
        for (int i = 0; i < m_vertices; i++)
        {
            cerr << " " <<  permV[i];
        }
        cerr << endl;
        //cerr << " s" << print_minlex_signature(true, 8, m_minlex_signature) << endl;



        vector<int> smallest_pv;
        ...
        if (sigvalid)
        {
            ...
            sig_best_valid = true;
            sig_best_length = sig_length;
        }

        for (int pv = p_from; pv < p_to; pv++)
        {
            bool sigtest_is_better = !sig_best_valid;

            if (usedV[pv]) continue;


#define ADD_TO_SIG_TEST(X) {  sig_test[id] = X; if (sigtest_is_better == false) { if (sig_test[id] > sig_best[id] || id > sig_best_length) goto endOfLoop; if (sig_test[id] < sig_best[id]) sigtest_is_better = true; } ++id; }

            int id = 0;

#ifdef G_COLORED_EDGES
            for (int u = 0; u < next_p; u++)
                ADD_TO_SIG_TEST(get_2edge(permV[u],pv));
#endif



            if (sigtest_is_better)
            {
                std::swap(sig_test, sig_best);
                smallest_pv.clear();
                sig_best_length = id;
                sig_best_valid = true;
                sigvalid = false;
            }
            smallest_pv.push_back(pv);

            endOfLoop:
            ;
        }

        if (smallest_pv.size() == 0 && sigvalid)
            return;

        bool sig_updated = false;
        if (sigvalid == false)
        {
            // Copy the best signature and move the pointer
            for (int i = 0; i < sig_best_length; i++)
            {
                sig[i] = sig_best[i];
            }
            sig_updated = true;
            sig_length = sig_best_length;
        }
        sig += sig_best_length;
        ...
        if (next_p+1 < m_vertices)
        {
            sig_best_valid = false;
            sig_best_length = 0;

            assert(smallest_pv.size() > 0);
            int rest_sig_length = sig_length-sig_best_length;
            for (int pv : smallest_pv)
            {

                permV[next_p] = pv;
                usedV[pv] = 1;

                extend_sig(sigvalid, sig, rest_sig_length, next_p+1, permV, usedV, sig_test, sig_best, buffer_size);
                usedV[pv] = 0;
        ...
        else
        {
            ...
            //cerr << "  " << print_minlex_signature(true, 8, m_minlex_signature) << endl;
        }

        if (next_p == 3)
        {
            //cerr << print() << endl;
            //cerr << "sig_best_length=" << sig_best_length << endl;
            //cerr << "*best_sig_length=" << *best_sig_length << endl;
            //cerr << "next_p=" << next_p << endl;
            //cerr << "m_vertices=" << m_vertices <<  " next_p=" << next_p <<  " Bsig_best_length=" << sig_best_length << " *best_sig_length=" << *best_sig_length << endl;
        }
    }

    void create_minlex_signature_WIP()
    {
#ifdef G_USE_LEXMIN_FOR_ISOMORPHISM
		...
		int permV[V];
        int usedV[V];
		for (int i = 0; i < V; i++) permV[i] = -1;
		for (int i = 0; i < V; i++) usedV[i] = 0;


        //int sig[SIGNATURE_LENGTH]; // this is one we are trying to create
        m_minlex_signature[0] = m_vertices;
        assert(numeric_limits<flag_t>::max() > m_Theta);
        m_minlex_signature[1] = m_Theta;

        const int buffer_size = 2*SIGNATURE_LENGTH;
        flag_t sig_TMPA[buffer_size];
        flag_t sig_TMPB[buffer_size];

        extend_sig(m_minlex_signature_valid, m_minlex_signature+2, m_minlex_signature_length, 0, permV, usedV, sig_TMPA, sig_TMPB, buffer_size);
        m_minlex_signature_length += 2;
        ...
        //cerr << "Created signature for " << endl;
        //cerr << print() << endl;
        //cerr << print_minlex_signature() << endl;
#endif
    }
    ...
    void create_minlex_signature(bool force_creating=true, flag *lexmin_copy = nullptr)
    {
        ...
        #ifdef   G_USE_LEXMIN_FOR_ISOMORPHISM_OLD
        create_minlex_signature_OLD();
        #else
        //cerr << "Creating signature for " << print() << endl;
...
#ifdef G_ORDERED_VERTICES
        cerr << "contains_as_subflag not correctly implemented for ordered graphs. " << endl;      
        assert(0);
#endif        
...
public:
    ...
    flag& operator=(const flag& f)
    {
        ...
        {
#ifdef  G_USE_LEXMIN_FOR_ISOMORPHISM_STR
            m_minlex_signature_str = f.m_minlex_signature_str;
#endif
...
// for fast isomorphism testing
    ...
    flag_t m_color_edge[V*(V-1)/2];  // adjacency matrix
...
// This may greatly speed-up generation.
        ...
        for (int color = 2; color < max_color+1; color++)
#else
...
#elif defined(G_USE_FIRST_EDGE_COLOR_FOR_BLOWUP_ONLY) || defined(G_TOURNAMENT)
...
inline double P_F1_F2_IN_H(const flag &F1, const flag &F2, const flag &H, bool return_only_good_maps=false)
{
    ...
    if (H.labeled_vertices_cnt() > 0)
    {
        ...
        #ifdef G_ORDERED_VERTICES
            cerr << "ERROR: Ordered vertices not implemented" << endl;
            assert(0);
        #endif        
    }
...
bool fc_cfactor(istream *istr, coefficient_double_str &result, ostream *ostr, stringstream &debug_ss)
{
    //int poistion = istr->tellg();
...
bool fc_cpower(istream *istr, coefficient_double_str &result, ostream *ostr, stringstream &debug_ss) {
    //int poistion = istr->tellg();
    ...
    if (ch == '^')
    {
        ...
        fc_cfactor(istr, power, ostr, debug_ss);
        if (ostr != NULL) (*ostr) << "}";
...
bool fc_cterm(istream *istr, coefficient_double_str &result, ostream *ostr, stringstream &debug_ss) {
    //int poistion = istr->tellg();
...
bool fc_cexpression(istream *istr, coefficient_double_str &result, ostream *ostr, stringstream *debug_ss)
{
    //int poistion = istr->tellg();
...
class struct_symmetric_antisymmetric
{
    public:
        struct_symmetric_antisymmetric()
        {
        ...
        vector<vector<vector< pair<pair<int,int>, int> > > > m_to_projection_symmetric;
        vector<vector<vector< pair<pair<int,int>, int> > > > m_to_projection_antisymmetric;

    void built_to_projection(vector<vector<vector< pair<pair<int,int>, int> > > > &to_projection,  vector<vector<int> > projection, int dimension)
    {
        // make to_projection of size dimension X dimension
        vector<vector< pair<pair<int,int>, int> > > tmp;
...
vector<struct_symmetric_antisymmetric >  g_csdp_projections;
...
bool g_symmetric_antisymmetric_projections_in_single_block = false; // use =true only for debugging I guess
...
void calculate_symmetric_antisymmetric_projections(  const vector<flag> &flags_of_one_type, struct_symmetric_antisymmetric &sas, int verbose_output)
{
...
void calculate_type_automorphism_action(int blockID,  const vector<flag> &flags_of_one_type, ostream  &ostr, int verbose_output)
{
    ...
    ostr << blockID << " " << type.m_Theta << " " << flags_of_one_type.size() << " " << automorphism_counter << endl;
    ostr << aabuffer.str();
}


int project_and_print_header(vector<int> &block_sizes, const vector<flag> &flags_of_one_type, int verbose_output, ostream* p_outfile_latex= NULL, ostream* p_outfile_tap = NULL, ostream* p_outfile_taa = NULL)
{
    if (g_symmetric_antisymmetric_projections == false)
    {
    ...
    if (g_csdp_projections[blockID].m_already_calculated == false)
    {
            ...
            if (isomorphism_class[i] == i)
            {
                ...
                for(int j = 0; j < (int)flags_of_one_type.size(); j++)
                {
                    ...
                    if (isomorphism_class[j] == isomorphism_class[i])
                    {
                        ...
                        antisym.push_back(-j);
                        //cerr << "aa " << antisym[0] << " " << antisym[1] << endl;
                        g_csdp_projections[blockID].m_antisymmetric.push_back(antisym);
                        //cerr << "bb" << endl;
                    }
                }
                if (verbose_output >= 2) cerr << endl;
                ...
                //cerr <<  sym.size() << endl;
                assert(blockID < (int)g_csdp_projections.size());
                g_csdp_projections[blockID].m_symmetric.push_back(sym);
                //cerr << "b" << endl;
                //cerr << "xx" << endl;
            }
        }
        ...
        */
        ...
        if (p_outfile_latex != NULL)
        {
            if (g_symmetric_antisymmetric_projections_in_single_block == true)
            {
                (*p_outfile_latex) << "\\item SDP Symmetric, anti-symmetric block $" << flags_of_one_type.size() << "\\times" << flags_of_one_type.size() << "$ \\\\ \\begin{enumerate}" << endl;
            }
            else
            {
            ...
            if (g_symmetric_antisymmetric_projections_in_single_block == false)
                (*p_outfile_latex) << "\\end{enumerate} \\item SDP Anti-Symmetric block $" << g_csdp_projections[blockID].m_antisymmetric.size() << "\\times" << g_csdp_projections[blockID].m_antisymmetric.size() << "$ \\\\ \\begin{enumerate}" << endl;
    ...
    //cerr << "SIZES " << symmetric.size() << " "  << antisymmetric.size() << " " << flags_of_one_type.size() << endl;
    assert(symmetric.size() + antisymmetric.size() == flags_of_one_type.size());

    if (g_symmetric_antisymmetric_projections_in_single_block == true)
    {
        block_sizes.push_back(flags_of_one_type.size());
        //ostr << symmetric.size() + antisymmetric.size() << " ";
        g_csdp_projections[blockID].m_use_projection = true;

        if (p_outfile_tap != NULL)
        {
            (*p_outfile_tap) << "# Symmetric and anticymmetric projection in one block." << endl;
            (*p_outfile_tap) << -(int)flags_of_one_type.size() << endl;
            //calculate_type_automorphism_partition(flags_of_one_type, *p_outfile_tap, verbose_output);
        }
...
template<typename T>
void print_projection(ostream &ostr, int matrixID, int blockID, map<pair<int, int>, T> &found_id_pairs, const vector<vector<int> > &projection)
{
/*
    if (matrixID == 2 && blockID == 21)
    {
        cerr << "blockID=" << blockID << endl;
        for (auto & xy_coeff : found_id_pairs)
        {
            if (xy_coeff.second != 0)
            {
                ostr.precision(G_PRECISION);
                cerr <<  matrixID << " " << blockID << " " << xy_coeff.first.first+1 << " " << xy_coeff.first.second+1 << " " << smart_round(xy_coeff.second) << endl;
            }
        }
    }
*/
    // And now we process the projection
    for (int i = 0; i < (int)projection.size(); i++)
    {
        for (int j = i; j < (int)projection.size(); j++)
        {
            double coefficient_ij = 0;
            for (int a : projection[i])
            {
                for (int b : projection[j])
                {
                    int ca = abs(a);
                    int cb = abs(b);
                    if (ca > cb) swap(ca, cb);
                    //map<pair<int, int>, T>::iterator ab_coeff = found_id_pairs.find({ca,cb});
                    auto ab_coeff = found_id_pairs.find({ca,cb});
                    if (ab_coeff != found_id_pairs.end())
                    {
                        if ((a >= 0 && b >= 0) ||  (a < 0 && b < 0))
                            coefficient_ij += ab_coeff->second;
                        else
                            coefficient_ij -= ab_coeff->second;
                    }
                }
            ...
            {
                ostr.precision(G_PRECISION);
                ostr << matrixID << " " << blockID << " " << i+1 << " " << j+1 << " " << smart_round(coefficient_ij) << endl;
            }
        }
    }
}
...
template<typename T>
void print_projection_using_to_projection(ostream &ostr, int matrixID, int blockID, map<pair<int, int>, T> &found_id_pairs, const vector<vector<vector< pair<pair<int,int>, int> > > > &to_projection, int diagonal_offest)
{
    map<pair<int, int>, T> id_pairs_in_projection;
    ...
    for(const auto &ids : found_id_pairs)
    {
        ...
        for (  const pair<pair<int,int>, int>  &to_projection : to_projection[x][y] )
        {
            int px = to_projection.first.first;
            int py = to_projection.first.second;
            int pcoeff = to_projection.second;
    ...
    {
        if (ids.second != 0)
            ostr << matrixID << " " << blockID << " " << ids.first.first+1+diagonal_offest << " " << ids.first.second+1+diagonal_offest << " " << ids.second << endl;
    }
}

template<typename T>
void project_and_print_map(ostream &ostr, int matrixID, int blockID, map<pair<int, int>, T> found_id_pairs)
{
    if (g_symmetric_antisymmetric_projections == false || g_csdp_projections[blockID].m_use_projection == false)
    {
        for(const auto &ids : found_id_pairs)
        {
    ...
    if (g_symmetric_antisymmetric_projections_in_single_block == true)
    {
        print_projection_using_to_projection(ostr, matrixID, blockID, found_id_pairs, g_csdp_projections[blockID].m_to_projection_symmetric, 0);
        print_projection_using_to_projection(ostr, matrixID, blockID, found_id_pairs, g_csdp_projections[blockID].m_to_projection_antisymmetric, g_csdp_projections[blockID].m_symmetric.size());
    }
    else
    {
        print_projection_using_to_projection(ostr, matrixID, blockID, found_id_pairs, g_csdp_projections[blockID].m_to_projection_symmetric, 0);
        print_projection_using_to_projection(ostr, matrixID, blockID+1, found_id_pairs, g_csdp_projections[blockID].m_to_projection_antisymmetric, 0);
    }
    ...
    if (g_symmetric_antisymmetric_projections_in_single_block == true)
    {
        vector<vector<int> > all_projections = g_csdp_projections[blockID].m_symmetric;
        for (auto &tmp : g_csdp_projections[blockID].m_antisymmetric) all_projections.push_back(tmp);

        print_projection(ostr, matrixID, blockID, found_id_pairs, all_projections);
    }
    else
    {
        print_projection(ostr, matrixID, blockID, found_id_pairs, g_csdp_projections[blockID].m_symmetric);
        print_projection(ostr, matrixID, blockID+1, found_id_pairs, g_csdp_projections[blockID].m_antisymmetric);
    }
    return;
    ...
    for (int typesize = Kn-2; typesize >= 0; typesize--)
    {
        ...
        for (int flagID : g_flag_SDP_symmetry_classes[symmetry_classID])
        {
...
#ifdef G_ORDERED_VERTICES
            for (int i = 0; i < typesize; i++) subset_split_type[i] = -1; // still same order
#else
            ...
            do
            {
                ...
                {
                    if (subset_split_type[i] == -1)
                    {
                        mapping_type[inType] = i;
                        type_basic |= 1 << inType++;
                    }
                }
#else
...
//        assert(0);
        return 0;
        //
#warning "-elcsq  may have troubles getting all types correctly"
#else
        flag g;
...
int g_flag_square_linear_constraints_blocks = 0;

vector<int> g_linear_constraintsSQ_blocks_to_SDPblocks;


int print_CSDP_linear_constraints_ALL_print_header(vector<int> &block_sizes, int Kn, int verbose_output, ostream* p_outfile_latex= NULL, ostream* p_outfile_tap = NULL, ostream* p_outfile_taa = NULL )
{
    if (verbose_output)
    {
...
//                        }
    ...
    if (g_use_square_linear_constraints)
    {
        ...
        for (int j = 0; j < (int)g_linear_constraints.size(); j++)
        {
            ...
            // TODO!!!! Maybe some types are missing. Check isomorphism as Daniel
...
//                    g_flag_square_linear_constraints_blocks += project_and_print_header(block_sizes, used_blocks+g_flag_square_linear_constraints_blocks, g_linear_constraintsSQ_Tsize_Tid_TsizeSQ_TSQid_Flag[Tsize][Tid][TsizeSQ][TSQid], verbose_output);
                    g_flag_square_linear_constraints_blocks += project_and_print_header(block_sizes, g_linear_constraintsSQ_Tsize_Tid_TsizeSQ_TSQid_Flag[Tsize][Tid][TsizeSQ][TSQid], verbose_output, p_outfile_latex, p_outfile_tap, p_outfile_taa);
                }
            }
...
#ifdef G_ORDERED_VERTICES
           for (int i = 0; i < Tsize; i++) subset_split_type[i] = -1; // still same order
#else
                            ...
                            for (int TsizeSQ = Tsize; TsizeSQ <= maxTsizeSQ; TsizeSQ++)
                            {
...
#ifdef G_ORDERED_VERTICES
                                for (int i = 0; i < TsizeSQtolabel; i++) subset_split_Fsq_type[i] = -1; // still same order
#else
                                ...
                                do {
                                    ...
                                    for (int i = 0; i < f.m_vertices; i++)
                                    {
                                        if (subset_split_type[i] == -1)
                                        {
                                            mapping_Fsq_type[inFsqType] = i;
                                            type_Fsq |= 1 << inFsqType++;
                                        }
                                        else
                                        {
                                            if (subset_split_F[FlcSplitID] == label_rest)
                                            {
                                                if (subset_split_Fsq_type[FsqTypeSplitID] == -1)
                                                {
                                                    mapping_Fsq_type[inFsqType] = i;
                                                    type_Fsq |= 1 << inFsqType++;
                                                }
                                    ...
                                    do
                                    {
                                        ...
                                        for (int i = 0; i < f.m_vertices; i++)
                                        {
                                            if (subset_split_type[i] == -1)
                                            {
                                                mapping_Fsq1[inFsq1] = i;
                                                mapping_Fsq2[inFsq2] = i;
                                                type_Fsq1 |= 1 << inFsq1++;
                                                type_Fsq2 |= 1 << inFsq2++;
                                            }
                                            else
    ...
    do {
        ...
        do
        {
            ...
            for (int typesizeFsq = typesize; typesizeFsq <= maxtypesizeFsq; typesizeFsq++)
            {
                ...
                // Select the type
#ifdef G_ORDERED_VERTICES
                for (int i = 0; i < typesizeFsqExtra; i++) subset_split_Fsq_type[i] = -1; // still same order
#else
...
int print_CSDP_constraints_header(int Kn, vector<int> &block_sizes, int verbose_output = 1, ostream* p_outfile_latex= NULL, ostream* p_outfile_tap = NULL, ostream* p_outfile_taa = NULL)
{
    ...
    print_CSDP_linear_constraints_ALL_print_header(block_sizes, Kn, verbose_output, p_outfile_latex, p_outfile_tap, p_outfile_taa);
#else
    if (g_linear_constraints.size() != 0)
    {
        if (g_use_simple_linear_constraints)
...
void print_CSDP_specific_part(int Kn, bool upper_bound = true, ostream &ostr = cout, int verbose_output = 1, ostream* p_outfile_latex= NULL, ostream* p_outfile_tap = NULL, ostream* p_outfile_taa = NULL)
{
    ...
    for (int f = 0; f < (int)g_flags.size(); f++)
    {
        ...
        //blocks+=
        project_and_print_header(block_sizes, g_flags[f], verbose_output, p_outfile_latex, p_outfile_tap, p_outfile_taa);
    ...
    //blocks +=
    print_CSDP_constraints_header(Kn, block_sizes, verbose_output, p_outfile_latex, p_outfile_tap, p_outfile_taa);
...
#ifdef G_USE_FIRST_EDGE_COLOR_FOR_BLOWUP_ONLY
        } while ( std::next_permutation(color_permutation+2,color_permutation+COLORS_EDGES));  // leave 0,1 in place
#else
...
\pgfmathtruncatemacro{\plusone}{#1+1}
    ...
    "\\draw  \\foreach \\x in {0,...,\\plusone}{(\\x,0) coordinate(x\\x)};} \n"
#else
    ...
    bool python_output, int verbose_output, bool use_weights_str)
{
    ...
    struct_symmetric_antisymmetric sas;
...
void find_extremal_vectors_in_blow_up_of_B_for_vector_of_flags_with_phantom(const flag &B, const flag &phantom, vector<flag> f_for_test, vector<int> &weights, vector<string> &weights_str, bool python_output, int verbose_output, bool use_weights_str)
{
    // Go over rooting of the type part of the mapping
    if (f_for_test.size() == 0)
    {
    ...
    while (mapping_Theta[Theta] == 0)
    {
        ...
        if (mapping_type.is_isomorphic_to(type) == false)
        {
            ...
            {
                phantom_type.as_subflag_in_blowup(phantom,mapping_Theta,Theta,Theta);
...
void find_extremal_vectors_in_blow_up_of_with_phantom(const flag &B, const flag &phantom, const string &weights_in_str, const string &weights_str_in_str, const string &colors_str, bool python_output, int verbose_output)
{
    vector<int> weights = find_extremal_vectors_read_weights(B, weights_in_str);
    vector<string> weights_str = find_extremal_vectors_read_weights_str(B, weights_str_in_str);
    find_extremal_vectors_process_colors(B, colors_str);
    ...
    //int i = 7;
    {
        ...
        //find_extremal_vectors_in_blow_up_of_B_for_type(B, i, weights, verbose_output);
        find_extremal_vectors_in_blow_up_of_B_for_vector_of_flags_with_phantom(B, phantom, g_flags[i], weights, weights_str, python_output, verbose_output, weights_str_in_str != "");
        //if (python_output) cout << "]";
    }
    if (python_output) cout << "]" << endl;
}
...
-o OUTPUT_DAT-S              Write sdp to a file OUTPUT_DAT-S, usable 'stdout', default test.dat-s
-ub                          Creates SDP computing UPPER BOUND on the objective
-lb                          Creates SDP computing LOWER BOUND on the objective
-fp                          Stores flag products or tries to reuse them - usefull for repeated computations, eats more space
-e FLAG                      Prints all extnesions of FLAG and exits
-ec FLAG                     Prints all extnesions of FLAG and # of labeled copies of FLAG in extensions and exits
-e_fc CFLAGFILE              Prints all extnesions with coefficients of flags with coefficients in the file
-ft FLAG                     Forbiden Test - checks if FLAG is forbidden
...
-tff                         Types From File for labeled flags (rather than generating) if possible.
                            Should be used with -fglf and without -fp.
-lff FLAGFILE                Override the name of the file for loading labeled flags
-fgc                         Force generating constraints - not loading from file
...
-du                          Dump unlabeled even when loaded from file - usefull for example if the input contain also forbidden flags, then this can work a 1 time filter
-dl                          Dump labeled even when loaded from file - usefull for example to have the order of flags 
-dug                         Dump unlabeled flags as they are being generated - usefull for example if you want to enumerate all flags and not really compute the rest
-dto                         Dump types in order and stop. Useful to checking what is the order of types when building CSDP program.
-gsul                        Generate small unlabled from large ones. Usefull if large ones generated externally and smaller are missing.
-qagu                        Quit after generating unlabeled flags
-qagl                        Quit after generating labeled flags
-oco                         Only Compute Objective
...
-csdp_pp PYTHONSCRIPT        When dat-s file is ready, a python script is called that may tweak the file. Useful for adding constraints.
-mosek                       use mosek rather than csdp
-sdpa                        use sdpa rather than csdp
-solver [mosek|sdpa|csdp|lpsdp]  use one of the solvers
-s SCALE                     when using lpsdp, use option -s SCALE passed to the solver
-omp N                       Number of threads to use. OMP_NUM_THREADS overwrite.
-ml G                        Memory Limit in GB. The program may be killed if the limit is crossed but not necessarily. Also passed to lpsdp. 
-latex                       Print problem formulation in latex and terminate execution
-latexx                      Print problem formulation in latex after expanding constraints and terminate execution
-latexSDP                    Print the SDP formulation in latex. Careful, it may be very very large
-elcs                        Enable Linear Constraints simple - just multiple with a variable.
-dlcs                        Disable Linear Constraints simple - just multiple with a variable.
-dlcp                        Disable Lienar Constraints Products.
-elcp                        Enable Linear Constraints products - multiplied with other flags.
-elcp2                       Enable Linear Constraints products by expanding first.
-elcsq                       Enable Linear Constraints times SQuare of flags.
-elcsp                       Enable Linear Constraints times Self Products of flags.
-tc  CFLAGFILE FILENAME      Test if linear constraints in  FILENAME are satiffied on CFLAGFILE.
-tce CFLAGFILE EFILENAME     Test if linear constraints in  EFILENAME are satiffied on CFLAGFILE.
-tcpn                        Print negative constraints.
-ac FILENAME                 Load Additional Constrains from FILENAME. Can be used multiple times.
-ace FILENAME                Load additional extended constrains from FILENAME. Can be used multiple times.
-acv FILENAME                Load additional constrains from FILENAME in vectorized form. Can be used multiple times.
-ab FILENAME                 Load Additional Blocks for CSDP from FILENAME. Can be used multiple times.
-lcdt                        Test linear constraints for duplicate with type permutaitons.
-lcad                        Allow duplicate constraints when loading, shoudl be faster and when you are sure you have no duplicates, it is good.
-dlc                         Dump linear constraints and quit. Usefull with -lcdt or -elcp2
-vc FILENAME                 Vectorize constraints, write in file FILENAME and quit.
-pcbq                        Print constraints blocks as in CSDP output and quit. Can be used as temp.
-pcbq_sf INT                 Start with processing unlabeled flag INT. Default is start from beginning (1). Used for resuming failed caluclations. INT means in the dat-s file line starting with INT will be first so be careful.
-nsf FILENAME                No slack Flags. When going for exact result, it might be useful to solve the
                            problem with some variables missing slack. This prodices CSDP where slack are not used for the specified flags.
-esap                        Enable symmetric-antisymmetric projections for semidefinite matrices.
-dsap                        Disable symmetric-antisymmetric projections for semidefinite matrices.
-ctap                        Calculate type automorphism projections for main types in sdp. It adds -dsap
-ctapa                       Calculate type automorphism projections for all things in the sdp, including -elcsq. It adds -dsap
-ctaas                       Calculate type automorphism actions for main types in sdp. It adds -dsap
-ctaa                        Calculate type automorphism actions for all things in the sdp, including -elcsq. It adds -dsap
-df  FILENAME                Draw flags in file FILENAME
-dfd FILENAME                Draw flags in file FILENAME where every flag has a density in front
-df1                         When drawing flags, make edges colored 1 with non-zero opacity.
-dfe                         Use \enumerate and \itemize when listing flags
-eo                          Extended Objective output when dumping flags - i.e. + and ratinal coefficients if possible.
-dfcout                      Use std::out when writing the drawing
-dtc FILENAME                Converting FILENAME that is dat-s file to cbf file. Useful when trying mosek instead of csdp
-cdin FILENAME               Compute densities in FILENAME. FILENAME contains flags with coefficients and
                            ...
                            would like to see what is the corresponding statistics for smaller flags.
-jtn  FILENAME               Jan to Nice - read list of flags in Jan Volec's format and write them in our nice one.
-jtnc FILENAME               Same as -jtn but flags have coefficients in  [    ].
-srp DOUBLE                  Smart round precision. When printing for CSDP, sometimes one wants all coefficients
                            to be integers but float calculations migh result in errors that need to be rounded.
-FisoH FLAG_F FLAG_H         Test isomorphism of F and H
-Fs FLAG_F                   Prints the isomorphims signature of F
-FsametypeH FLAG_F FLAG_H    Test is F and H have the same type
-FidenticalH FLAG_F FLAG_H   Test if F and H are identical and writes why if they are not
-FinH  FLAG_F FLAG_H         Counts density of F in H including unlabeling
-F1F2inH FLAG_F1 FLAG_F2 FLAG_H Counts density of F1 and F2 in H (debug function)
-FinbupH FLAG_F FLAG_H       Counts density of F in a blow-up of H including unlabeling. In the blow-up
                            vertices in the blow-up are connected by color colored by 1
-FEinbupH EXPRESSION FLAG_H  Counts density of EXPRESSION in a blow-up of H including unlabeling. In the blow-up
                            vertices in the blow-up are connected by color colored by 1
-eFinbupH FLAG_F FLAG_H      Test if F exists in a blow-up of H.
-cF1timesF2 C FLAG_F1 FLAG_F2 Computes product of flags F1 and F2 and muptiplies by double C. Same type is needed.
-gbe                         Generate Baber's Equalities and quit
-glft N TYPE                 Generate Labeled Flags on N verties of type TYPE
-fev FLAG_CONSTRUCTION       Find Extremal Vectors if we assume that blow-up of FLAG_CONSTRUCTION is the right thing.
                            Usefull for rounding. Default is G_BLOW_UP_COLOR_EDGES=1
                            Projection to symmetric X antisymmetric follows -esap/-dsap
-fevw WEIGHTS                Adds to -fev also weights for every vertex. Default
                            weights are everywhere 1. Should be integeres.
-fevws WEIGHTSSTR            Adds to -fev also weights for every vertex. Default
                            is no extra weights. Note these weights are strings! Useful for irrational stuff.
-fevc COLORS                 Adds to -fev also colors for every vertex. Default
                            colors are everywhere 1. Should be integeres.
-fevfvec                     Print the output as vector of flags rather than as vector for python.
-fevphantom FLAG_CONSTRUCTION  construction for phantom edges where types can be rooted
-fpv CFLAGFILE               Find projection vectors. All should have smae types. TODO
-tt CFLAGFILE BLOWUP         Test Tight - test if the combination of flags is tight for extremal example that would be the blowup
                            can be combined with -fevw or -fevc
-tttm MAPPING                In Test Tight, prescribes mapping of Theta - so only one is tested rather than all possible.
-ds                          Disable symmetry usage if the program was compiled with it
-fdvi FLAG_CONSTRUCTION      Find density Vectors if we assume that iterated blow-up of FLAG_CONSTRUCTION is the right thing.
                            Usefull for checking if the dual vector of CSDP solution is close to the iterated construction.
-fdv  FLAG_CONSTRUCTION      Find density Vectors if we assume that blow-up of FLAG_CONSTRUCTION is the right thing.
                            It uses the same parameters as -fev
                            Usefull for checking if the dual vector of CSDP solution is close to the iterated construction.
-gsn N  FLAG(s)              Generates all subflags on N vertices from the input
-fflp  FLAGFILE                Lexicographical permutation of flags in FLAGFILE
-fflpc CFLAGFILE               Lexicographical permutation of flags in CFLAGFILE
-ffrd FILEFLAGS_ALL          Filter input list of flags to remove duplicates.
-ffdt FILEFLAGS_ALL          Duplicate Test - find the first pair that are duplicates.
-ffa FILEFLAGS_ALL FILEFLAGS_ALLOWED  Filter input list of flags to include only flags in the second file.
-fff FILEFLAGS_ALL FILEFLAGS_FORBIDDEN  Filter input list of flags to include only flags not in the second file.
-ffac CFLAGFILE FILEFLAGS_ALLOWED  Same as -ffa but input has coefficients.
-fffc CFLAGFILE FILEFLAGS_FORBIDDEN  Same as -fff but input has coefficients.
-ffas FILEFLAGS_ALL FILEFLAGS_ALLOWED  Filter input list of flags to include as subflag at least one of the flags in the second file.
-fffs FILEFLAGS_ALL FILEFLAGS_FORBIDDEN  Filter input list of flags to include only flags without subflags in the second file.
-ffasc CFLAGFILE FILEFLAGS_ALLOWED    Same as -ffas but input has coefficients.
-fffsc CFLAGFILE FILEFLAGS_FORBIDDEN  Same as -fffs but input has coefficients.
-fc_addCtoF C  FLAGFILE              Flag Calculator: Loads flags from FLAGFILE and prints them with all with coefficient C
-fc_delCfromF CFLAGFILE            Flag Calculator: Loads flags and coefficients from CFLAGFILE and prints flags without coefficients
-fc_CtimesF C  CFLAGFILE             Flag Calculator: Loads flags and coefficients from CFLAGFILE and multiplies by C.
-fc_F1timesF2 CFLAGFILE1 CFLAGFILE2  Flag Calculator: Loads flags and coefficients from CFLAGFILE1 and CFLAGFILE and mulitplies.
-fc_squareF   CFLAGFILE              Flag Calculator: Loads flags and coefficients from CFLAGFILE and squares.
-fc_F1plusF2 CFLAGFILE1 CFLAGFILE2   Flag Calculator: Loads flags and coefficients from CFLAGFILE1 and CFLAGFILE and adds them.
-fc_unlabelF CFLAGFILE               Flag Calculator: Loads flags and coefficients from CFLAGFILE and unlabels them.
-fc_uplabelF CFLAGFILE TYPE          Flag Calculator: Loads flags and coefficients from CFLAGFILE and uplabels them to TYPE.
-fc_cleanF   CFLAGFILE               Flag Calculator: Loads flags and coefficients from CFLAGFILE and simplifies.
-fc_cleanFe  CFLAGFILE EPS           Flag Calculator: Loads flags and coefficients from CFLAGFILE and simplifies,  del entries <= EPS.
-fc_F1inF2 CFLAGFILE1 FLAGFILE2      Flag Calculator: Calculates CFLAGFILE1 in FLAGFILE2. Like F1 being objective and F2 all on k vertices.
-fc_maxF1inF2 CFLAGFILE1 FLAGFILE2   Flag Calculator: Calculates max CFLAGFILE1 in FLAGFILE2. Prints also partial results.
-p pNAME DOUBLE                      Flag Calculator: replaces parameters while reading input by doubles. Names must start with p
-fcm EXPRESSION                      Flag Calculator: simple calculator with matrices
-fcmd EXPRESSION                     Flag Calculator: draw simple calculator with matrices
-fc EXPRESSION                       Flag Calculator: simple calculator
-fcd EXPRESSION                      Flag Calculator: draw simple calculator
    )"
<< "Filenames: " << endl
...
<< "    " << filename_prefix() << "__forbidden.txt       forbidden flags" << endl
 ...
 << "    " << filename_prefix() << "__vertexblind_permutations.txt allowed perms" << endl
    ...
    while (i < argc)
    {
        if (string(argv[i]) == "-help") {
            ...
            if (solver_str == "lpsdp") use_lpsdp = true;
...
//// TODO: Make this parallel
    ...
    if (find_extremal_vectors)
    {
        ...
        find_extremal_vectors_in_blow_up_of_with_phantom(find_extremal_vectors_construction, find_extremal_vectors_construction_phantom, find_extremal_vectors_weights, find_extremal_vectors_weights_str, find_extremal_vectors_colors, find_extremal_vectors_python_output, verbose_output);
   ...
   if (dump_types_in_order)
   {
    ...
    if (print_constraints_blocks_and_quit)
    {
        ...
        cout << Kn << endl;
        vector<int> block_sizes;
        cout << print_CSDP_constraints_header(Kn, block_sizes, verbose_output) << endl;
